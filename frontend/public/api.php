...
if ($requestMethod === 'GET' && $action === 'posts') {
    echo supabaseRequest('GET', 'posts?select=*,source:sources(name)&deleted=eq.false&limit=2000');
    exit;
}
...
if ($requestMethod === 'GET' && $action === 'posts-trash') {
    echo supabaseRequest('GET', 'posts?select=*,source:sources(name)&deleted=eq.true&limit=2000');
    exit;
}
