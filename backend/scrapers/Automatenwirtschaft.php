...
class Automatenwirtschaft {
    public function fetch() {
        $sourceName = 'Automatenwirtschaft';
        ...
        while ($page <= $maxPages) {
            ...
            $entries = $xpath->query('//div[contains(@class,"blog-list-item")]');
            ...
            foreach ($entries as $entry) {
                $titleNode = $xpath->query('.//a[contains(@class,"blog-list-title")]', $entry)->item(0);
                $dateNode = $xpath->query('.//div[contains(@class,"blog-list-date")]', $entry)->item(0);

                if (!$titleNode || !$dateNode) {
                    echo "Unvollständiger Eintrag, überspringe.\n";
                    continue;
                }
