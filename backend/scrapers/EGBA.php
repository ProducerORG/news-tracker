...
class EGBA {
    public function fetch() {
        ...
        while ($page <= $maxPages) {
            ...
            $entries = $xpath->query('//div[contains(@class,"news-row")]');
            ...
            foreach ($entries as $entry) {
                $titleNode = $xpath->query('.//h3[contains(@class,"news-title")]', $entry)->item(0);
                $linkNode = $xpath->query('.//a[contains(@class,"news-title-link")]', $entry)->item(0);
                $dateNode = $xpath->query('.//p[contains(@class,"publish-date")]', $entry)->item(0);

                if ($titleNode && $linkNode && $dateNode) {
                    $title = trim($titleNode->textContent);
                    $href = $linkNode->getAttribute('href');
                    $link = (strpos($href, 'http') === 0) ? $href : 'https://www.egba.eu' . $href;
